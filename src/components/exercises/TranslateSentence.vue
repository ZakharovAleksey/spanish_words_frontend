<template>
  <div>
    <p class="text-left title mb-2"><b>Translate this sentence:</b></p>
    <div>
      <v-text-field
          variant="outlined"
          :readonly="true"
      >{{ exercise_data['sentence_to_translate'] }}
      </v-text-field>
      <v-text-field
          v-model="user_input"
      ...
      <div v-if="show_error_message">
        ...
        >
          <div class="red-text">
            ...
            <b>Correct Answer:</b> {{ exercise_data['correct_translation'] }}
          </div>
        </v-alert>
        <v-alert
            v-if="is_user_input_correct"
...
export default {
  ...
  computed: {
    is_user_input_correct() {
      return compareSpanishWords(this.user_input, this.exercise_data['correct_translation'])
    }
  },
  methods: {
    notifyParent() {
